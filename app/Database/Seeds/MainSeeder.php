...
class MainSeeder extends Seeder
{
    public function run()
    {
        ...
        $this->db->table('user_menu')->insertBatch([
            ...
            ['id' => 9, 'menu_category' => 4, 'menu_category_id' => 4, 'title' => 'Settings',        'url' => 'users',           'icon' => 'settings',      'is_active' => 1],
        ]);
        echo "✔ Menus seeded.\n";

        // 5. Seed User Access (Superadmin gets access to everything)
        $this->db->table('user_access')->truncate();
        $accessData = [];

        // Menu category access
        for ($i = 1; $i <= 4; $i++) {
            $accessData[] = ['role_id' => 1, 'menu_category_id' => $i, 'menu_id' => 0];
        }

        // Menu access
        for ($i = 1; $i <= 9; $i++) {
            $accessData[] = ['role_id' => 1, 'menu_category_id' => 0, 'menu_id' => $i];
        }
